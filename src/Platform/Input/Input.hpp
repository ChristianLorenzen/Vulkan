...
public:
    ...
    void checkMovement(GLFWwindow *window, Faye::Camera *camera, double delta)
    {
        ...
        camera->rotation.x = glm::clamp(
            camera->rotation.x - static_cast<float>(mouseDelta.y) * 0.01f,
            -1.5f,
            1.5f);
        camera->rotation.y = glm::mod(
            camera->rotation.y - static_cast<float>(mouseDelta.x) * 0.01f,
            glm::two_pi<float>());

        glm::vec3 move{0.0f};
        if (glfwGetKey(window, keyMap.forward) == GLFW_PRESS)
        {
            move += Faye::Camera::forwardFromRotation(camera->rotation);
        }
        ...
        {
            move -= Faye::Camera::rightFromRotation(camera->rotation);
        }
        if (glfwGetKey(window, keyMap.right) == GLFW_PRESS)
        {
            move += Faye::Camera::rightFromRotation(camera->rotation);
        }
        if (glfwGetKey(window, keyMap.up) == GLFW_PRESS)
        {
            move += Faye::Camera::upFromRotation(camera->rotation);
        }
        if (glfwGetKey(window, keyMap.down) == GLFW_PRESS)
        {
            move -= Faye::Camera::upFromRotation(camera->rotation);
        }
        ...
        {
            camera->position += static_cast<float>(delta) * editorCameraSettings.moveSpeed * glm::normalize(move);
        }
