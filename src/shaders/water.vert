...
void main()
{
    ...
    WaveParams waves[4];
    waves[0]=WaveParams(normalize(vec2(1.,.5)),.35,9.,1.,.28);
    waves[1]=WaveParams(normalize(vec2(-.4,1.)),.22,5.5,1.2,.22);
    waves[2]=WaveParams(normalize(vec2(.3,-.8)),.14,3.2,1.5,.18);
    waves[3]=WaveParams(normalize(vec2(-.7,.3)),.08,2.,1.8,.14);
    
    // Transform to world space — Gerstner is evaluated here so wavelengths are metres.
    vec3 worldBase=(push.modelMatrix*vec4(vtx.pos,1.)).xyz;
    ...
    vec3 priorWorldBase=(push.priorModelMatrix*vec4(vtx.pos,1.)).xyz;
    vec3 priorPos=priorWorldBase;
    vec3 priorTangent=vec3(1.,0.,0.);// derivatives unused for position
    vec3 priorBinormal=vec3(0.,0.,1.);
    for(int i=0;i<4;i++)
    {
    ...
    fragCurrentClip=ubo.projection*ubo.view*vec4(pos,1.);
    fragPriorClip=ubo.priorViewProjection*vec4(priorPos,1.);
