...
const float kLineWidthPixels = 2.0;

float log10f(float x) { return log(x) * 0.4342944819; } // 1 / ln(10)
...
float lineCoverage(vec2 planeUv, vec2 footprint, float cell)
{
    ...
    vec2 coverage = vec2(1.0) - clamp(distanceToLine / max(footprint, vec2(1e-8)), 0.0, 1.0);
...
void main()
{
    ...
    if (abs(denominator) < 1e-6 || t < 0.0 || t > 1.0)
        discard;
    ...
    float lod = max(0.0,
        log10f(footprintLength * max(push.minPixelsBetweenCells, 0.5) / baseCell) + 1.0);
    float lodBlend = fract(lod);

    float cellFine = baseCell * pow(10.0, floor(lod));
    float cellMid = cellFine * 10.0;
    float cellCoarse = cellMid * 10.0;
    ...
    else
    {
        ...
        coverage = coverageFine * (1.0 - lodBlend);
    }
    ...
    float distanceFade = 1.0 - saturatef(radialDistance / max(push.maxDistance, 1e-3));
    ...
    vec4 clipPos = ubo.projection * ubo.view * vec4(hitWorld, 1.0);
